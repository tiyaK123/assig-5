...
ThreadedBSTree::ThreadedBSTree() { root = nullptr; }

// destructor will delete individual nodes
ThreadedBSTree::~ThreadedBSTree() { clear(root); }
 ...
 * 		     returns true
 * **/
bool ThreadedBSTree::removeSearch(TreeNode *parent, TreeNode *tr,
                                  const int &item) {
  if (tr == nullptr) {
    root = nullptr;
    return false;
...
void ThreadedBSTree::removeNode(TreeNode *node, TreeNode *parent) {
  ...
  // node has two child
    ...
    } else { // Node is right Child of its parents
